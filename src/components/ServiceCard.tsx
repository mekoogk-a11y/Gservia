...
export const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    <div 
      ...
      className="group relative flex flex-col justify-between bg-slate-900 rounded-3xl p-5 sm:p-6 border border-slate-800 shadow-lg hover:shadow-2xl hover:border-blue-500/80 transition-all duration-200 hover:-translate-y-1 cursor-pointer text-start"
    >
      <div>
        {/* Top Header */}
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-3">
            <img 
              src={service.logoUrl} 
              alt={service.name} 
              className="w-12 h-12 rounded-2xl object-contain bg-white p-1.5 shadow-sm shrink-0 group-hover:scale-105 transition-transform" 
            />
            <div>
          ...
          <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
            {service.matchScore && service.matchScore > 80 && (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-950 text-blue-300 border border-blue-600/50 flex items-center gap-1">
        ...
        <div className="flex flex-wrap items-center gap-1.5 mb-4">
          {service.freePlan ? (
            <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-emerald-950/70 text-emerald-300 border border-emerald-800/80">
          ...
          {service.languages.includes('ar') && (
            <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-purple-950/70 text-purple-300 border border-purple-800/80">
              🇸🇦 {isArabic ? 'واجهة عربية' : 'Arabic UI'}
            </span>
          )}

          <span className="px-2 py-0.5 rounded-lg text-[10px] font-medium bg-slate-950 text-slate-400 border border-slate-800">
            {service.userLevel[0]}
          </span>
        </div>
      </div>
      ...
      <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1 text-xs font-bold text-slate-300">
          ...
          <span className="text-[10px] text-slate-500 font-normal">({service.reviewCount})</span>
        </div>
        ...
        <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
          <button
            onClick={() => onSelectService(service)}
            className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-bold transition-colors border border-slate-700 flex items-center gap-1"
          >
